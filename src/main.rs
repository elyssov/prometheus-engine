...
fn draw_text(mesh: &mut ChunkMesh, s: &str, centre: Vec3,
             right: Vec3, up: Vec3, cell: f32, color: [f32; 4],
             normal_front: [f32; 3], normal_back: [f32; 3]) {
    ...
    let mut push_quad = |c: [Vec3; 4]| {
        ...
        for p in c {
            mesh.vertices.push(MeshVertex {
                position: [p.x, p.y, p.z], normal: normal_front, color, material: 0,
            });
        }
        ...
        for p in [c[0], c[3], c[2], c[1]] {
            mesh.vertices.push(MeshVertex {
                position: [p.x, p.y, p.z], normal: normal_back, color, material: 0,
            });
        }
...
impl App {
    fn new() -> Self {
        ...
        Self {
            ...
            cat_vy: 0.0, cat_grounded: true, debug_collision: false,
            paused: false, cursor_locked: false,
            screenshot_pending: false, screenshot_index: 0, screenshot_flash_t: 0.0,
    ...
    fn update_cat(&mut self, dt: f32) {
        ...
        if self.keys.w { mv += cat_fwd; }
        if self.keys.s { mv -= cat_fwd; }
        if self.keys.d { mv -= cat_right; }   // strafe right
        if self.keys.a { mv += cat_right; }   // strafe left
        ...
        for v in &self.vox_objects {
            ...
            if next.x < mn.x - CAT_COLLIDER_RADIUS * 0.5
                ...
                || next.z > mx.z + CAT_COLLIDER_RADIUS * 0.5 { continue; }
            let top = mx.y;
            if top < 8.0 { continue; }
            if top > floor_y && top <= prev_y + 5.0 { floor_y = top; }
        }
        ...
        next.y = new_y;
        next = resolve_cat_collision(next, &self.apartment, &self.vox_objects,
                                      CAT_COLLIDER_RADIUS, self.debug_collision);
        self.cat_pos = next;
    ...
    fn pose_cat_skeleton(&mut self) {
        ...
        self.cat_sk.bone_mut("neck").local_rotation =
            Quat::from_rotation_x(head_pitch * 0.35);
        self.cat_sk.bone_mut("head").local_rotation =
            Quat::from_rotation_x(head_pitch * 0.65);
    ...
    fn try_swipe_hit(&mut self) {
        ...
        let pick = match (brick_hit, vox_hit) {
            ...
            (Some(bh), Some(vh)) => if bh.1 <= vh.1 { Some(("brick", bh.0, bh.1)) }
                                                    else { Some(("vox",   vh.0, vh.1)) },
        };
        ...
        if kind == "vox" {
            ...
            let (color, gib_he, total_destroyed, annihilate_now, sev, applied, backlash) = {
                ...
                if !h.applied {
                    ...
                    (vox.color, Vec3::splat(vox.vox_size * 0.5), false, false,
                     h.severity, false, bl)
                } else {
                    ...
                    if annihilate {
                        ...
                        let world_r = (h.effective_radius * vox.vox_size * 0.5).max(vox.vox_size * 1.2);
                        vox.carve_sphere(impact, world_r, &mut removed);
                    }
                    (vox.color, Vec3::splat(vox.vox_size * 0.5),
                     vox.is_empty(), annihilate, h.severity, true, 0.0)
                }
            };
            if !applied {
                self.cat_hp = (self.cat_hp - backlash).max(0.0);
                println!("  🐾 OW! vox too tough — backlash {:.2}, cat hp {:.1}",
            ...
            println!("  💥 carve vox @ {:.0}cm → sev={:?} dust={} {}{}",
                dist, sev, removed.len(),
        ...
        if let Some((idx, dist)) = Some((idx, dist)) {
            ...
            let brick_dur   = self.apartment.bricks[idx].durability
            ...
            if !h.applied {
                ...
                self.cat_hp = (self.cat_hp - backlash).max(0.0);
                println!("  🐾 OW! {} too tough — paw backlash {:.2}, cat hp {:.1}/{:.1}",
            ...
            let vox_idx = if let Some(&v) = self.brick_to_vox.get(&idx) {
                ...
                let vox = VoxObject::from_brick(
                    brick_pos, brick_rot, brick_he, brick_color, brick_mat,
                    brick_dur, 32);
            ...
            let (gib_he, totally_destroyed, annihilation) = {
                ...
                if annihilation {
                    ...
                    let world_radius = (h.effective_radius * vox.vox_size * 0.5).max(vox.vox_size * 1.2);
                    vox.carve_sphere(impact, world_radius, &mut removed_positions);
                }
                (Vec3::splat(vox.vox_size * 0.5), vox.is_empty(), annihilation)
            };
            ...
            println!("  💥 SWIPE! {} @ {:.0}cm → sev={:?} dust={} {}{}",
                name, dist, h.severity, removed_positions.len(),
    ...
    fn apply_cursor_grab(&mut self) {
        ...
        if want_lock == self.cursor_locked { return; }
        if let Some(w) = &self.win {
            if want_lock {
                let r = w.set_cursor_grab(CursorGrabMode::Locked)
    ...
    fn build_pause_overlay(&self, eye: Vec3, center: Vec3) -> ChunkMesh {
        ...
        for p in c {
            mesh.vertices.push(MeshVertex {
                position: [p.x, p.y, p.z], normal: n_front, color: outer_color, material: 0,
            });
        }
        mesh.indices.push(base);     mesh.indices.push(base + 1); mesh.indices.push(base + 2);
        mesh.indices.push(base);     mesh.indices.push(base + 2); mesh.indices.push(base + 3);
        ...
        for p in [c[0], c[3], c[2], c[1]] {
            mesh.vertices.push(MeshVertex {
                position: [p.x, p.y, p.z], normal: n_back, color: outer_color, material: 0,
            });
        }
        mesh.indices.push(base2);    mesh.indices.push(base2 + 1); mesh.indices.push(base2 + 2);
        mesh.indices.push(base2);    mesh.indices.push(base2 + 2); mesh.indices.push(base2 + 3);
        ...
        draw_text(&mut mesh, "PAUSED", title_centre, right, up, 0.40,
                  title_color, n_front, n_back);

        // Hint "ESC TO RESUME" smaller, below.
        let hint_centre = billboard_centre - up * 1.8;
        draw_text(&mut mesh, "ESC TO RESUME", hint_centre, right, up, 0.20,
                  hint_color, n_front, n_back);
    ...
    fn build_shot_flash(&self, eye: Vec3, center: Vec3) -> ChunkMesh {
        ...
        draw_text(&mut mesh, "SHOT", badge_centre, right, up, 0.18,
                  title_color, n_front, n_back);
        mesh
    }
    ...
    fn spawn_gibs(&mut self, pos: Vec3, dir_bias: Vec3, color: [u8; 3],
                  half: Vec3, n: usize, base_speed: f32) {
    ...
    fn update_apartment_physics(&mut self, dt: f32) {
        ...
        for i in 0..n {
            ...
            if my_min_y <= 0.5 { continue; } // resting on floor
            let my_he = Vec3::new(
                b.half_extents.x * b.scale.x,
                b.half_extents.y * b.scale.y,
                b.half_extents.z * b.scale.z,
            ...
            for j in 0..n {
                ...
                if (s_top - my_min_y).abs() > 2.5 { continue; }
                if s.world_position.x + s_he.x < my_min_x { continue; }
                if s.world_position.x - s_he.x > my_max_x { continue; }
                if s.world_position.z + s_he.z < my_min_z { continue; }
                if s.world_position.z - s_he.z > my_max_z { continue; }
                supported = true;
                break;
            }
            ...
            for v in &self.vox_objects {
                if v.is_empty() { continue; }
                let (mn, mx) = v.world_aabb();
                if (mx.y - my_min_y).abs() > 2.5 { continue; }
                if mx.x < my_min_x || mn.x > my_max_x { continue; }
                if mx.z < my_min_z || mn.z > my_max_z { continue; }
                supported = true;
                break;
            }
            if supported { continue; }
    ...
    fn update_gibs(&mut self, dt: f32) {
        for g in self.gibs.iter_mut() {
            ...
            if g.pos.y < floor_y {
                g.pos.y = floor_y;
                if g.vel.y < 0.0 { g.vel.y = -g.vel.y * GIB_BOUNCE; }
    ...
    fn build_crosshair_mesh(&self, eye: Vec3) -> ChunkMesh {
        ...
        let on_target = self.apartment.raycast_breakable(probe_origin, probe_dir, probe_dist).is_some()
    ...
    fn render(&mut self) {
        ...
        if let Some(device) = &self.device {
            ...
            if any_dirty || self.vox_gpu.is_none() {
                ...
                for v in self.vox_objects.iter_mut() {
                    if v.is_empty() { v.dirty = false; continue; }
                    let m = v.build_mesh();
                    let base = combined.vertices.len() as u32;
                    combined.vertices.extend_from_slice(&m.vertices);
                    for ix in &m.indices { combined.indices.push(base + ix); }
        ...
        let mut encoder = device.create_command_encoder(&Default::default());
        {
            ...
            for mesh in [&self.apartment_gpu, &self.vox_gpu, &self.cat_gpu, &self.gibs_gpu,
                         &self.crosshair_gpu, &self.pause_overlay_gpu, &self.shot_flash_gpu] {
                if let Some(m) = mesh {
...
fn ray_aabb_box(origin: Vec3, dir: Vec3, min: Vec3, max: Vec3, max_dist: f32) -> Option<f32> {
    ...
    if tmax >= tmin.max(0.0) && tmin <= max_dist { Some(tmin.max(0.0)) } else { None }
}
...
fn resolve_cat_collision(pos_in: Vec3, apt: &BrickModel, vox: &[VoxObject], radius: f32, debug: bool) -> Vec3 {
    let mut pos = pos_in;
    ...
    for _ in 0..3 {
        ...
        for b in &apt.bricks {
            ...
            if max_y <= slab_lo || min_y >= slab_hi { continue; }
            ...
            if d2 < r2 - 1e-4 {
                any = true;
                if debug { debug_blockers.push(b.name.clone()); }
                if d2 > 1e-4 {
                    let d = d2.sqrt();
                    let push = radius - d;
                    pos.x += dx / d * push;
        ...
        for v in vox {
            if v.is_empty() { continue; }
            let (mn, mx) = v.world_aabb();
            if mx.y <= slab_lo || mn.y >= slab_hi { continue; }
            ...
            if d2 < r2 - 1e-4 {
                any = true;
                if debug { debug_blockers.push(format!("vox#{}", vox.iter().position(|x| std::ptr::eq(x, v)).unwrap_or(0))); }
                if d2 > 1e-4 {
                    let d = d2.sqrt();
                    let push = radius - d;
                    pos.x += dx / d * push;
                    ...
                    let m = pen_xp.min(pen_xn).min(pen_zp).min(pen_zn);
                    if m == pen_xp      { pos.x = mx.x + radius; }
                    else if m == pen_xn { pos.x = mn.x - radius; }
                    else if m == pen_zp { pos.z = mx.z + radius; }
                    else                { pos.z = mn.z - radius; }
                }
            }
        }
        if !any { break; }
    ...
    if debug && !debug_blockers.is_empty() {
        ...
        if pushed > 0.05 {
            ...
            println!("  🐛 cat ({:.0},{:.0}) pushed {:.1}cm by [{}]",
                pos.x, pos.z, pushed, debug_blockers.join(", "));
        }
    }
    pos
}
    ...
    for b in apt.bricks.iter_mut() {
        ...
        let (dur, tag) = if lname.contains("bulb") || lname.contains("mirror")
            ...
            (Durability::glass(), "glass")
                  ...
                  || lname.contains("porcelain") || lname.contains("toilet")
                  || lname.contains("sink") || lname.contains("tub") {
            (Durability::ceramic(), "ceramic")
        } else if lname.contains("pillow") || lname.contains("blanket")
                  || lname.contains("linen") || lname.contains("rug")
                  || lname.contains("towel") || lname.contains("curtain")
                  || lname.contains("jacket") || lname.contains("mattress")
                  || lname.contains("sofa") || lname.contains("cushion")
                  || lname.contains("bed_frame_mattress") {
            (Durability::fabric(), "fabric")
        } else if lname.contains("fridge") || lname.contains("stove")
                  || lname.contains("metal") || lname.contains("oven") {
            ...
            WindowEvent::KeyboardInput { event, .. } => {
                ...
                match event.physical_key {
                    PhysicalKey::Code(KeyCode::Escape) if pressed => {
                        ...
                        println!("  {} (Esc again to {})",
                            if self.paused { "⏸ PAUSED — menu open" } else { "▶ Resumed" },
            ...
            WindowEvent::MouseInput { state, button, .. } => {
                ...
                match button {
                    winit::event::MouseButton::Left => {
                        match self.mode {
                            Mode::Cat => if pressed { self.keys.space_edge = true; }
...
fn main() {
    ...
    println!("    Space         — JUMP");
    println!("    Right mouse   — hold for free orbit (cat stands still); release snaps camera back");
